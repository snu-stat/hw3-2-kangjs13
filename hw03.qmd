...
GHCR 푸시를 위해 `permissions`에 `packages: write`를 추가하였다. `MASS`는 R의 recommended 패키지로 함께 제공되므로 CRAN 설치 목록에서는 제외하였다. `Dockerfile`에서 `_site/hw03.ipynb`를 복사하므로 Docker 빌드는 반드시 Quarto 렌더링 뒤에 실행되어야 한다.
