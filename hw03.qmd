...
`.github/workflows/publish.yml`은 다음과 같이 작성한다. 숙제 2의 HTML 및 ipynb 생성, GitHub Pages 배포 단계를 유지하고, 이번 과제에서 필요한 패키지를 추가한 뒤 마지막에 GHCR 로그인과 Docker 이미지 빌드/푸시 단계를 추가한다.
  ...
  build-deploy:
    ...
    steps:
      ...
      - name: Install Python dependencies via Conda
        run: |
          conda install -n r-reticulate numpy pandas matplotlib polars scipy statsmodels scikit-learn -y
        # 추가로 필요한 패키지 설치
        shell: bash -l {0}
      ...
      - name: Install R Packages
        ...
        run: |
          install.packages(c("reticulate", "knitr", "rmarkdown", "dplyr", "ggplot2", "broom"))
        # 추가로 필요한 패키지 설치
        shell: Rscript {0}
...
GHCR 푸시를 위해 `permissions`에 `packages: write`를 추가하였다. 이미지 태그는 `ghcr.io/${{ github.repository }}:latest`로 지정하여 `ghcr.io/snu-stat/hw3-2-kangjs13:latest`처럼 저장소별로 고유한 패키지 이름을 사용한다. 이렇게 하면 조직 안의 다른 저장소가 같은 `my-r-env` 패키지 이름으로 푸시할 때 생기는 `write_package` 권한 충돌을 피할 수 있다. `MASS`는 R의 recommended 패키지로 함께 제공되므로 CRAN 설치 목록에서는 제외하였다. `Dockerfile`에서 `_site/hw03.ipynb`를 복사하므로 Docker 빌드는 반드시 Quarto 렌더링 뒤에 실행되어야 한다.
